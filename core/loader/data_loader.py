...
class patch_loader(data.Dataset):
    ...
    """
    def __init__(self, split='Colab Notebooks', stride=30 ,patch_size=99, is_transform=True,
                 augmentations=None):
        self.root = 'drive/'
        self.split = split
        ...
        if 'test' not in self.split: 
            ...
            self.seismic = self.pad_volume(np.load(pjoin('drive', 'Colab Notebooks','train_seismic.npy')))
            self.labels = self.pad_volume(np.load(pjoin('drive', 'Colab Notebooks','train_labels.npy')))
        elif 'test1' in self.split:
            self.seismic = np.load(pjoin('drive', 'Colab Notebooks','test1_seismic.npy'))
            self.labels = np.load(pjoin('drive', 'Colab Notebooks','test1_labels.npy'))
        elif 'test2' in self.split:
            self.seismic = np.load(pjoin('drive', 'Colab Notebooks','test2_seismic.npy'))
            self.labels = np.load(pjoin('drive', 'Colab Notebooks','test2_labels.npy'))
        else:
        ...
        if 'test' not in self.split:
            ...
            for split in ['train', 'val', 'train_val']:
                ...
                path = pjoin('drive', 'Colab Notebooks', 'patch_' + split + '.txt')
...
class section_loader(data.Dataset):
    ...
    """
    def __init__(self, split='Colab Notebooks', is_transform=True,
                 augmentations=None):
        self.root = 'drive/'
        self.split = split
        self.is_transform = is_transform
        ...
        if 'test' not in self.split: 
            # Normal train/val mode
            self.seismic = np.load(pjoin('drive', 'Colab Notebooks','train_seismic.npy'))
            self.labels = np.load(pjoin('drive', 'Colab Notebooks','train_labels.npy'))
        elif 'test1' in self.split:
            self.seismic = np.load(pjoin('drive', 'Colab Notebooks','test1_seismic.npy'))
            self.labels = np.load(pjoin('drive', 'Colab Notebooks','test1_labels.npy'))
        elif 'test2' in self.split:
        ...
        if 'test' not in self.split:
            ...
            for split in ['train', 'val', 'train_val']:
                # reading the file names for 'train', 'val', 'trainval'""
                path = pjoin('drive', 'Colab Notebooks', 'section_' + split + '.txt')
                file_list = tuple(open(path, 'r'))
